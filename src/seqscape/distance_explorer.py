...
def run(args) -> None:
    ...
    af_widget_dir_text = getattr(args, "umap_explorer_dir", "") or sample_summary.get("umap_explorer_dir", "") or sample_summary.get("af_widget_dir", "")
    ...
    (outdir / "distance_explorer.html").write_text(
        html_template(json.dumps(payload, separators=(",", ":")), "Distance Explorer"),
        encoding="utf-8",
    )
    with (outdir / "summary.txt").open("w") as fh:
        ...
        if af_widget_dir_text:
            fh.write(f"umap_explorer_dir\t{Path(af_widget_dir_text).resolve()}\n")
